/*---------------------------------------------------------------------
 $Header: /Perl/OlleDB/OlleDB.xs 59    05-11-13 16:33 Sommar $
...
// C<use MSSQL::OlleDB> for the first time.
//======================================================================
...
// Called when a Perl script says C<use MSSQL::OlleDB>.
void initialize ()
{
   SV *sv;
   DWORD       err;
   ...
   if (FAILED(ret)) {
      ...
      croak("MSSQL::OlleDB, which requires MDAC 2.6\n");
   }

   // Set Version string.
   if (sv = perl_get_sv("MSSQL::OlleDB::Version", TRUE))
   {
        char buff[256];
        sprintf(buff, "This is MSSQL::OlleDB, version %s\n\nCopyright (c) 2005 Erland Sommarskog\n",
                XS_VERSION);
                            ...
                            BOOL   props_debug)
{
  ...
  for (int i = 0; gbl_init_props[i].propset_enum != not_in_use; i++) {
   ...
   if (too_old_sqloledb) {
      ...
      warn("version of SQLOLEDB. To use MSSQL::OlleDB you must have at least\n");
...
// but OlleDB can use it for its own errors too.
                            ...
                            BSTR    msg)
{
   msg_handler(olle_ptr, msgno, state, severity, msg,
               NULL, NULL, 0, NULL, L"MSSQL::OlleDB", 1, 1);
}
      ...
      // we will survive the ordeal.
         ...
         // An SQL error detected by SQLOLEBB.
            ...
            // Eh? Missing locale? Whatever, don't drop it on the floor.
               ...
               BSTR source = SysAllocString(L"MSSQL::OlleDB");
       ...
       if (sv_service != NULL) {
          ...
          property[no_of_props].vValue.bstrVal  =
              (sv_message != NULL ? SV_to_BSTR(sv_message) :
                             SysAllocString(L"Query notification set by OlleDB"));
          no_of_props++;
       }
                 ...
                 SV   *sv_rows_affected)
{
    ...
    // Set rowset properties from OlleDB options.
...
//======================================================================
MODULE = MSSQL::OlleDB           PACKAGE = MSSQL::OlleDB
