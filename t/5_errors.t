#---------------------------------------------------------------------
# $Header: /Perl/OlleDB/t/5_errors.t 12    05-11-13 21:46 Sommar $
#
...
use strict;
use MSSQL::OlleDB qw(:DEFAULT :consts);
...
$expect_print =
    ['=~ /^Message -1.+MSSQL::OlleDB.+Severity:? 10/',
     "=~ /Mandatory part 'Month' missing/",
     "=~ /Message from MSSQL::OlleDB at/",
     '=~ /^Message -1.+MSSQL::OlleDB.+Severity:? 10/',
     "=~ /Could not convert Perl value.+smalldatetime/",
     "=~ /Message from MSSQL::OlleDB at/",
     '=~ /^Message -1.+MSSQL::OlleDB.+Severity:? 16/',
...
$expect_print =
    ['=~ /^Message -1.+MSSQL::OlleDB.+Severity:? 10/',
     ...
     "=~ /Message from MSSQL::OlleDB at/",
     q'=~ /^Message -1.+MSSQL::OlleDB.+Severity:? 10/',
     q!=~ /Procedure '#date_sp' .+ parameter.+'\@(notpar|hugo)'/!,
     "=~ /Message from MSSQL::OlleDB at/",
     q'=~ /^Message -1.+MSSQL::OlleDB.+Severity:? 16/',
