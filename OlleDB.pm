#---------------------------------------------------------------------
# $Header: /Perl/OlleDB/OlleDB.pm 40    05-11-13 16:33 Sommar $
#
...
use constant PACKAGENAME => 'MSSQL::OlleDB';
...
sub FETCH {
   ...
   if (not exists $myattrs{$key}) {
       ...
       $key =~ s/^./uc($&)/e;
       if (not exists $myattrs{$key}) {
           $self->olle_croak("Attempt to fetch a non-existing OlleDB property '$key'");
       }
   }
...
sub STORE {
   ...
   if (not exists $myattrs{$key}) {
       $key =~ s/^./uc($&)/e;
       if (not exists $myattrs{$key}) {
           $self->olle_croak("Attempt to set a non-existing OlleDB property '$key'");
       }
   }
...
sub sql_message_handler {
    ...
    if ($ErrInfo->{SaveMessages}) {
       my %message;
       tie %message, 'MSSQL::OlleDB::ErrInfo::Messages';
       %message = (Errno    => $errno,
                   State    => $state,
                   Severity => $severity,
                   Text     => $text,
    ...
    unless ($ErrInfo->{NeverPrint}{$errno} or $ErrInfo->{NeverPrint}{$sqlstate}) {
       ...
       if ($print_msg) {
          if (not $source) {
             ...
             print $fh "Internal OlleDB call: $procedure\n" if $procedure;
          }
       }
...
sub new_err_info {
    ...
    my(%ErrInfo);
    tie %ErrInfo, 'MSSQL::OlleDB::ErrInfo';
   ...
   foreach my $parname (sort keys %$hashparams) {
      ...
      if ($parname !~ /^\@/) {
         if (exists $$hashparams{'@' . $parname}) {
            my $msg = "Warning: hash parameters for MSSQL::OlleDB::sql " .
...
package MSSQL::OlleDB::ErrInfo::Messages;
